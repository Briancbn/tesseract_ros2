  ...
  : root_link_(root_link), topic_namespace_(topic_namespace), clock_(std::make_shared<rclcpp::Clock>(RCL_ROS_TIME))
{
  ...
  arrows_pub_ = node_->create_publisher<visualization_msgs::msg::MarkerArray>("/display_arrows", 1);
  axes_pub_ = node_->create_publisher<visualization_msgs::msg::MarkerArray>("/display_axes", 1);
...
void ROSPlotting::plotMarker(const tesseract_visualization::Marker& marker, std::string ns)
{
  switch (marker.getType())
  {
    case static_cast<int>(tesseract_visualization::MarkerType::ARROW):
    {
      ...
      auto arrow_marker_msg = getMarkerArrowMsg(marker_counter_, root_link_, topic_namespace_, clock_->now(), m);
    ...
    case static_cast<int>(tesseract_visualization::MarkerType::AXIS):
    {
      ...
      visualization_msgs::msg::MarkerArray msg =
          getMarkerAxisMsg(marker_counter_, root_link_, topic_namespace_, clock_->now(), m.axis, m.getScale());
    ...
    case static_cast<int>(tesseract_visualization::MarkerType::CONTACT_RESULTS):
    {
      ...
      {
        visualization_msgs::msg::MarkerArray msg =
            getContactResultsMarkerArrayMsg(marker_counter_, root_link_, topic_namespace_, clock_->now(), m);
