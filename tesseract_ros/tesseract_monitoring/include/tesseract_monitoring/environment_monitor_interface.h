...
{
public:
  typedef std::shared_ptr<EnvironmentMonitorInterface> Ptr;
  typedef std::shared_ptr<const EnvironmentMonitorInterface> ConstPtr;
   ...
   */
  bool wait(rclcpp::Duration timeout = rclcpp::Duration(-1)) const;
   ...
   * @return True if namespace is available, otherwise false
   */
  bool waitForNamespace(const std::string& monitor_namespace, rclcpp::Duration timeout = rclcpp::Duration(-1)) const;
  ...
  static tesseract_environment::Environment::Ptr getEnvironment(const std::string& monitor_namespace)
  {
    ...
    res->flags = tesseract_msgs::srv::GetEnvironmentInformation::COMMAND_HISTORY;

    auto result_future = client->async_send_request(res);
    if (rclcpp::spin_until_future_complete(node_, result_future) !=
      rclcpp::FutureReturnCode::SUCCESS)
    {
      RCLCPP_ERROR_STREAM(node_->get_logger(), "getEnvironment: Failed to get monitor environment information!");
      return nullptr;
    }
