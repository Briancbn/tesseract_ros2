...
const std::string LOGGER_ID{"env_monitor_interface"};
...
bool EnvironmentMonitorInterface::wait(ros::Duration timeout) const
{
  if (ns_.empty())
  {
    RCLCPP_ERROR(rclcpp::get_logger(LOGGER_ID), "TesseractMonitorInterface namespaces are empty and cannot wait!");
    return false;
  }
...
bool EnvironmentMonitorInterface::waitForNamespace(const std::string& monitor_namespace, ros::Duration timeout) const
{
  ...
  while (ros::ok())
  {
    ...
    if (results)
    {
      tesseract_msgs::msg::GetEnvironmentInformation res;
      res.request.flags = tesseract_msgs::msg::GetEnvironmentInformationRequest::COMMAND_HISTORY;
                                               ...
                                               const tesseract_environment::Command& command) const
{
  ...
  {
    RCLCPP_ERROR(rclcpp::get_logger(LOGGER_ID), "[%s]: Failed to convert latest changes to message and update monitored environment!", monitor_namespace);
    return false;
  }
}
...
{
  tesseract_msgs::msg::ModifyEnvironment res;
  ...
  if (!status || !res.response.success)
  {
    RCLCPP_ERROR(rclcpp::get_logger(LOGGER_ID), "[%s]: sendCommands: Failed to update monitored environment!", ns);
    return false;
  }
...
{
  tesseract_msgs::msg::GetEnvironmentInformation res;
  res.request.flags = tesseract_msgs::msg::GetEnvironmentInformationRequest::JOINT_STATES |
                      tesseract_msgs::msg::GetEnvironmentInformationRequest::LINK_TRANSFORMS |
                      tesseract_msgs::msg::GetEnvironmentInformationRequest::JOINT_TRANSFORMS;

  bool status = ros::service::call(R"(/)" + monitor_namespace + DEFAULT_GET_ENVIRONMENT_INFORMATION_SERVICE, res);
  if (!status || !res.response.success)
  {
    RCLCPP_ERROR(rclcpp::get_logger(LOGGER_ID), "[%s]: getEnvironmentState: Failed to get monitor environment information!", monitor_namespace);
    return nullptr;
  }
